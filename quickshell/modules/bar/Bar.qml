...
Item {
  ...
  function checkPopout(y: real): void {
    ...
    if (y >= oy && y <= oy + o.implicitHeight) {
      popouts.currentName = "";
      popouts.currentCenter = Qt.binding(() => oy + o.implicitHeight / 2);
      popouts.detach("winfo")
    //   popouts.currentName = "osicon";
    //   popouts.currentCenter = Qt.binding(() => oy + o.implicitHeight / 2);
    ...
    } else if (y >= cy && y <= cy + c.implicitHeight + spacing){
      popouts.currentName = "clock";
      popouts.currentCenter = Qt.binding(() => cy + c.implicitHeight / 2);
      popouts.hasCurrent = true;
  ...
  Item {
    ...
    implicitWidth: Math.max(osIcon.implicitWidth, workspaces.implicitWidth, activeWindow.implicitWidth, tray.implicitWidth, clock.implicitWidth, statusIcons.implicitWidth)
